...
export async function listInvitationsForInternalAdmin(): Promise<
    InternalInviteRow[]
> {
    const root = path.join(process.cwd(), "data", "clientes");
    const out: InternalInviteRow[] = [];
    const tipos = await fs
    ...
    for (const t of tipos) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                const panelId =
                    typeof rsvpPanel.panelId === "string" &&
                    rsvpPanel.panelId.trim()
                        ? rsvpPanel.panelId.trim()
                        : null;
                const panelEnabled =
                    Boolean(rsvpPanel.enabled) && Boolean(panelId);
                const baseToken =
                ...
                out.push({
                    ...
                    tokenHashPresent:
                        typeof access.tokenHash === "string" &&
                        access.tokenHash.length > 0,
                    allowLegacyUntil:
                        typeof access.allowLegacyUntil === "string" &&
