...
- `data/landing/configurador-es.json`
  - Textos en **español** del configurador (`/configurador`). El inglés sigue en código (`app/configurador/strings.ts`).

- `data/landing/landing-2.json` + `data/landing/landing-2.en.json`
  - Landing **oficial** en la **raíz** `/` (componente `LandingPageHome`).
  - Textos, `theme`, `pageLayout`, secciones; precios principales desde `data/landing/pricing.json`.
...
- Si queres cambiar precios:
  1) cambia ARS en `data/landing/pricing.json`
  2) ajusta `usdArs`
