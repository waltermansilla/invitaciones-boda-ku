  ...
  return (
    <div className="min-h-screen bg-[#faf9f7]">
      <header className="px-5 py-8 text-center text-white" style={{ backgroundColor: primaryColor }}>
        <h1 className="text-lg font-semibold tracking-[0.2em] uppercase">Listado de Invitados</h1>
        {diasRestantes !== null && diasRestantes > 0 && <p className="mt-2 text-sm font-light opacity-90">Faltan {diasRestantes} días para el evento</p>}
      </header>

      <div className="px-5 py-6">
        <div className="mb-3"><div className="rounded-lg bg-white px-4 py-6 text-center shadow-sm"><p className="text-4xl font-bold text-neutral-700">{total}</p><p className="mt-1 text-xs uppercase tracking-wide text-neutral-500">En total</p></div></div>
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-lg px-3 py-4 text-center" style={{ backgroundColor: "#f5d5d5" }}><p className="text-2xl font-bold" style={{ color: "#8b6b6b" }}>{stats.noAsisten}</p><p className="mt-1 text-[10px] uppercase tracking-wide" style={{ color: "#8b6b6b", opacity: 0.8 }}>Inasistencias</p></div>
        </div>
      </div>
...
function InvitadoRow({ invitado, isLast, copiedId, giftCardEnabled, primaryColor, expanded, onToggleExpand, onCopyLink, onDelete, onEdit, onTogglePago, onConfirmManual }: { invitado: Invitado; isLast: boolean; copiedId: string | null; giftCardEnabled: boolean; primaryColor: string; expanded: boolean; onToggleExpand: () => void; onCopyLink: (inv: Invitado) => void; onDelete: (id: string) => void; onEdit: () => void; onTogglePago: (inv: Invitado) => void; onConfirmManual: (inv: Invitado, estado: "confirmado" | "no_asiste") => void }) {
  ...
  if (invitado.tipo === "familia" && invitado.integrantes?.length) {
    ...
    if (estados.size > 1) { iconBg = "#e5e5e5"; iconTxt = "#666" }
    else if (estados.has("confirmado")) { iconBg = estadoBg.confirmado; iconTxt = estadoText.confirmado }
    else if (estados.has("no_asiste")) { iconBg = estadoBg.no_asiste; iconTxt = estadoText.no_asiste }
  }
    ...
    <div className={!isLast ? "border-b border-neutral-100" : ""}>
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer" onClick={onToggleExpand}>
        ...
        {invitado.tipo === "familia" && invitado.integrantes?.length ? (
          <div className="flex gap-1">{(() => { const e = { confirmado: 0, pendiente: 0, no_asiste: 0 }; invitado.integrantes!.forEach((i) => e[i.estado]++); return <>{e.confirmado > 0 && <span className="rounded px-2 py-1 text-[10px] font-medium" style={{ backgroundColor: estadoBg.confirmado, color: estadoText.confirmado }}>{e.confirmado}</span>}{e.pendiente > 0 && <span className="rounded px-2 py-1 text-[10px] font-medium" style={{ backgroundColor: estadoBg.pendiente, color: estadoText.pendiente }}>{e.pendiente}</span>}{e.no_asiste > 0 && <span className="rounded px-2 py-1 text-[10px] font-medium" style={{ backgroundColor: estadoBg.no_asiste, color: estadoText.no_asiste }}>{e.no_asiste}</span>}</> })()}</div>
        ) : (
          <span className="rounded px-2 py-1 text-[10px] font-medium uppercase" style={{ backgroundColor: bgColor, color: txtColor }}>{invitado.estado === "confirmado" ? "Confirmado" : invitado.estado === "no_asiste" ? "No asiste" : "Pendiente"}</span>
        )}
      </div>
      {expanded && (
        <div className="border-t border-neutral-100 bg-neutral-50 px-4 py-3">
          <div className="flex flex-wrap gap-2">
            ...
            {invitado.estado === "pendiente" && invitado.tipo !== "integrante" && <><button onClick={(e) => { e.stopPropagation(); onConfirmManual(invitado, "confirmado") }} className="flex items-center gap-1 rounded-lg bg-emerald-100 px-3 py-2 text-xs font-medium text-emerald-700"><Check className="h-3 w-3" />Confirmar</button><button onClick={(e) => { e.stopPropagation(); onConfirmManual(invitado, "no_asiste") }} className="flex items-center gap-1 rounded-lg bg-red-100 px-3 py-2 text-xs font-medium text-red-700"><X className="h-3 w-3" />No asiste</button></>}
          ...
          {invitado.tipo === "familia" && invitado.integrantes?.length && (
            <div className="mt-3"><p className="mb-2 text-xs font-medium text-neutral-700">Integrantes:</p><div className="space-y-1">{invitado.integrantes!.map((i) => (<div key={i.id} className="flex items-center justify-between rounded bg-white px-2 py-1"><span className="text-xs text-neutral-700">{i.nombre}</span><div className="flex items-center gap-2">{i.restricciones && <span className="flex items-center gap-1 text-[10px] text-neutral-500"><Utensils className="h-3 w-3" />{i.restricciones}</span>}<span className="rounded px-2 py-0.5 text-[10px] font-medium" style={{ backgroundColor: estadoBg[i.estado], color: estadoText[i.estado] }}>{i.estado === "confirmado" ? "OK" : i.estado === "no_asiste" ? "No" : "?"}</span></div></div>))}</div></div>
          )}
        </div>
      )}
    </div>
