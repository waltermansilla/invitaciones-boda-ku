...
export default function HeroSection({
    ...
    const useCustomPosition =
        namesPositionX !== undefined || namesPositionY !== undefined;
    ...
    const namesLetterSpacing =
        letterSpacingMap[namesDisplay?.letterSpacing || "wide"] || "0.2em";
    ...
    if (useTextsArray && namesDisplay?.texts) {
        namesDisplay.texts.forEach((t) => {
            if (t.font && !fontsToLoad.includes(t.font))
                fontsToLoad.push(t.font);
        });
    }
    ...
    const NamesOverlay = () => {
        ...
        const getPositionStyle = (): CSSProperties => {
            if (useCustomPosition) {
                return {
                    position: "absolute",
                    top:
                        namesPositionY !== undefined
                            ? `${namesPositionY}%`
                            : "50%",
                    left:
                        namesPositionX !== undefined
                            ? `${namesPositionX}%`
                            : "50%",
        ...
        const getPositionClass = () => {
            ...
            if (namesPosition === "top")
                return "absolute inset-x-0 top-0 pt-10";
            return isFloatingOverlay
                ? "absolute inset-x-0 bottom-0 pb-20"
                : isOverlayLayout
                  ? "absolute inset-x-0 bottom-0 pb-24"
                  : "absolute inset-x-0 bottom-0 pb-10";
        };

        if (namesLogo) {
            return (
                <div
                    className={`${getPositionClass()} flex flex-col items-center`}
            ...
            return (
                ...
                >
                    ...
                    <span
                        ...
                        style={{ color: namesColor, opacity: 0.6 }}
                    >
                        {separator}
                    </span>
                    {renderNameText(
                        namesDisplay.texts[1],
                        legacyFont,
                        legacyWeight,
                        legacySize,
                        legacyStyle,
