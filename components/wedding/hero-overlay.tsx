...
export default function HeroOverlay({
    ...
    const numPosition =
        typeof buttonPosition === "string" && !isNaN(Number(buttonPosition))
    ...
    const separatorTrackingClass =
        separator && separator.trim().length > 1 ? "tracking-[0.3em]" : "";

    const { first: nameFirst, second: nameSecond } = coupleNamesDisplayPair(
        brideName,
        groomName,
        nameOrder,
    );
    const showSecondNameLine = Boolean(nameSecond.trim());
    ...
    const nameSizeClass = isPixelSize
        ? ""
        : sizeClasses[nameSize] || sizeClasses["4xl"];
    const nameSizeStyle: React.CSSProperties = isPixelSize
        ? { fontSize: nameSize }
        : {};
                        ...
                        {showNames && (
                            <>
                                ...
                                {separator && separatorPos && (
                                    <span
                                        className={`absolute text-2xl font-extralight text-primary/60 sm:text-3xl md:text-4xl ${separatorTrackingClass}`}
                                        style={{
