...
import { LandingTdyHeader, type LandingTdyLocale } from "@/components/landing/landing-tdy-header"
import FooterSection from "@/components/wedding/footer-section"
import pricingData from "@/data/pricing.json"
import landingThemeData from "@/data/landing-theme.json"
...
function TdyIcon({
  ...
  return <I size={size} strokeWidth={1.75} className={className} style={color ? { color } : undefined} aria-hidden />
}
...
export interface LandingTdyData {
  ...
  sections: {
    ...
    incluye: {
      ...
      title: string
      subtitle: string
      listStyle?: { fontSizePx?: number; gapPx?: number; iconSize?: number }
      imageSrc: string
      imageAlt: string
    ...
    comparativa: {
      ...
      digital: {
        ...
        badge: string
        savingsLine: string
        bullets: { text: string; subtext?: string; icon: string }[] | string[]
        fromLabel: string
        fromPrice: string
        fromPriceUsd?: string
        footnote: string
    ...
    estilos: {
      ...
      items: { image?: string; videoSrc?: string; titulo: string; descripcion: string; href: string }[]
    }
    idiomas: {
      title: string
      subtitle: string
...
const THEME_DEFAULTS: Partial<LandingTdyTheme> = {
  typography: {
    ...
    priceFont: 'var(--font-landing-price), "Cormorant Garamond", Georgia, serif',
  ...
  buttons: {
    ...
    floatingBoxShadow: "0 2px 6px rgba(26, 25, 23, 0.06), 0 6px 18px rgba(26, 25, 23, 0.09)",
  },
  comparativa: {
...
function formatUsd(amount: number, locale: LandingTdyLocale, variant: "USD" | "US$" = "USD"): string {
...
function applyPricingToLandingData(raw: LandingTdyData, locale: LandingTdyLocale): LandingTdyData {
  ...
  const secondLangLabel = locale === "en" ? `+$${secondLangArs.toLocaleString("en-US")}` : `+$${secondLangArs.toLocaleString("es-AR")}`
  ...
  const paperTotalUsd = paperLines.reduce((sum, ars) => sum + toUsdPerItem(ars), 0)
  const savingsArs = Math.max(0, paperTotalArs - premiumArs)
  const savingsUsd = Math.max(0, paperTotalUsd - premiumUsd)

  const paperLineLabelsEs = ["Diseño", "Impresión (130 u.)", "Sobres y sellos", "Envío postal"]
  const paperLineLabelsEn = ["Design", "Printing (130 pcs)", "Envelopes & seals", "Postal shipping"]
  const paperLineLabels = locale === "en" ? paperLineLabelsEn : paperLineLabelsEs
  ...
  if (locale === "en") {
    next.sections.comparativa.paper.lines = paperLineLabels.map((label, i) => ({
      label,
      value: formatUsdMain(toUsdPerItem(paperLines[i] ?? 0)),
    }))
    next.sections.comparativa.paper.totalValue = formatUsdMain(paperTotalUsd)
    ...
    next.sections.comparativa.paper.totalValue = formatArs(paperTotalArs, locale)
    next.sections.comparativa.paper.totalValueUsd = formatUsd(paperTotalUsd, locale, usdVariant)
  }
  if (locale === "en") {
    next.sections.comparativa.digital.fromPrice = formatUsdMain(premiumUsd)
    next.sections.comparativa.digital.fromPriceUsd = formatArs(premiumArs, locale)
  } else {
    next.sections.comparativa.digital.fromPrice = formatArs(premiumArs, locale)
    next.sections.comparativa.digital.fromPriceUsd = formatUsd(premiumUsd, locale, usdVariant)
  }
  ...
  const premiumPlan = next.sections.servicio.planes.find((p) => p.name.toLowerCase().includes("premium"))
  const uniquePlan = next.sections.servicio.planes.find((p) => p.name.toLowerCase().includes("diseño") || p.name.toLowerCase().includes("unique"))
  if (premiumPlan) {
    if (locale === "en") {
    ...
    premiumPlan.features = premiumPlan.features.map((f) => {
      if (locale === "en") {
        if (f.toLowerCase().includes("extra section")) {
          return f.replace(/\(\+ARS [\d,]+ per extra section\)|\(\+\$[\d,]+ per extra section\)/, `(+ARS ${pricingData.configurator.extraSection.toLocaleString("en-US")} per extra section)`)
        }
        if (f.toLowerCase().includes("second language")) {
          return f.replace(/\(\+ARS [\d,]+\)|\(\+\$[\d,]+\)/, `(+ARS ${secondLangArs.toLocaleString("en-US")})`)
        }
        return f
      }
      if (f.toLowerCase().includes("sección extra")) {
        return f.replace(/\(\+\$[\d.]+ por sección extra\)/, `(+$${pricingData.configurator.extraSection.toLocaleString("es-AR")} por sección extra)`)
      }
...
function mergeTheme(t: LandingTdyData["theme"]): LandingTdyTheme {
  ...
  return {
    ...base,
    typography: { ...THEME_DEFAULTS.typography!, ...(base.typography ?? {}) },
    text: { ...THEME_DEFAULTS.text!, ...(base.text ?? {}) },
    buttons: { ...THEME_DEFAULTS.buttons!, ...(base.buttons ?? {}) },
    comparativa: { ...THEME_DEFAULTS.comparativa!, ...(base.comparativa ?? {}) },
...
function blockRevealStyle(revealed: boolean, delayMs: number): React.CSSProperties {
...
function useSectionReveal() {
  ...
  const io = TDY_MOTION.sectionInView
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ...
      { threshold: io.threshold, rootMargin: io.rootMargin }
    )
    obs.observe(el)
...
function CtaLink({
  ...
  if (btn.type === "anchor") {
    return (
      <a
        href={btn.anchor || "#"}
        onClick={(event) => handleLocalAnchorClick(event, btn.anchor || "#")}
        className={className}
        style={style}
      >
        {btn.text}
...
function handleLocalAnchorClick(event: React.MouseEvent<HTMLAnchorElement>, anchor: string) {
...
function FloatingCta({
  ...
  const sectionIds = useMemo(() => {
    ...
    const idFromAnchor = data.anchor?.startsWith("#") ? normalized(data.anchor) : undefined
    const merged = [...idsFromArray, idFromSingle, idFromAnchor, "planes"].filter((id): id is string => Boolean(id))
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        ...
        const shouldHide = Array.from(visibility.values()).some(Boolean)
  ...
  const l1 = fc.line1
  const l2 = fc.line2
  const anchorTargetId = data.anchor?.startsWith("#") ? data.anchor.replace(/^#/, "") : undefined
      ...
      <div className="w-full overflow-hidden" style={{ borderRadius: r }}>
        <a
          ...
          style={{
            ...
            transitionDelay: slideIn ? `0s, ${fc.delayOpacity}, 0s` : "0s, 0s, 0s",
            WebkitBackfaceVisibility: "hidden",
            willChange: slideIn ? "auto" : "transform, opacity, filter",
          }}
        >
          <span
            aria-hidden
          ...
          >
            <Snowflake
              ...
              size={14}
              strokeWidth={1.8}
              style={{ left: "8%", top: "16%", color: b.floatingText, opacity: 0.92 }}
            />
            <Snowflake
              className="landing-fab-star--b absolute"
              size={10}
              strokeWidth={2.15}
              style={{ right: "10%", top: "22%", color: b.floatingText, opacity: 0.78 }}
            />
            <Snowflake
              className="landing-fab-star--c absolute"
              size={9}
              strokeWidth={2.1}
              style={{ left: "22%", bottom: "14%", color: b.floatingText, opacity: 0.72 }}
            />
            <Snowflake
              className="landing-fab-star--d absolute"
              size={13}
              strokeWidth={1.9}
              style={{ right: "18%", bottom: "18%", color: b.floatingText, opacity: 0.88 }}
            />
            <Snowflake
              className="landing-fab-star--a absolute"
              size={8}
              strokeWidth={2.2}
              style={{ left: "36%", top: "12%", color: b.floatingText, opacity: 0.66 }}
            />
            <Snowflake
              className="landing-fab-star--b absolute"
              size={11}
              strokeWidth={1.85}
              style={{ right: "34%", top: "15%", color: b.floatingText, opacity: 0.74 }}
            />
            <Snowflake
              className="landing-fab-star--c absolute"
              size={7}
              strokeWidth={2.25}
              style={{ left: "14%", bottom: "28%", color: b.floatingText, opacity: 0.62 }}
            />
            <Snowflake
              className="landing-fab-star--d absolute"
              size={9}
              strokeWidth={2.1}
              style={{ right: "8%", bottom: "30%", color: b.floatingText, opacity: 0.7 }}
            />
          </span>
          <span className="relative z-[1] block">
            <span
              ...
              style={{
                opacity: slideIn ? 1 : 0,
                transform: slideIn ? "translateY(0)" : `translateY(${l1.translateY}px)`,
                transitionProperty: "opacity, transform",
                transitionDuration: `${l1.durationOpacity}, ${l1.durationTransform}`,
                transitionTimingFunction: `${e}, ${e}`,
                transitionDelay: slideIn ? `${l1.delay}, ${l1.delay}` : "0s, 0s",
              }}
            >
            ...
            <span
              ...
              style={{
                ...
                opacity: slideIn ? 1 : 0,
                transform: slideIn ? "translateY(0)" : `translateY(${l2.translateY}px)`,
                transitionProperty: "opacity, transform",
                transitionDuration: `${l2.durationOpacity}, ${l2.durationTransform}`,
                transitionTimingFunction: `${e}, ${e}`,
                transitionDelay: slideIn ? `${l2.delay}, ${l2.delay}` : "0s, 0s",
              }}
            >
              {data.line2}
            </span>
...
function heroHoldMsForPhrase(phrase: string) {
  const normalized = phrase
    ...
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
  return normalized.includes("momento unico") ? HERO_HOLD_MS + HERO_MOMENTO_EXTRA_HOLD_MS : HERO_HOLD_MS
}

function HeroStaggerPhrase({ phrases, color }: { phrases: string[]; color: string }) {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setPhase("out"), heroHoldMsForPhrase(currentPhrase))
  ...
  useEffect(() => {
    ...
    const phrase = list[index]
    const t = window.setTimeout(() => setPhase("idle"), heroStaggerTotal(phrase.length))
    return () => clearTimeout(t)
  ...
  return (
    <span className={`${HERO_ROTATING_LINE_CLASS} motion-reduce:transition-none`} aria-live="polite">
      {chars.map((ch, i) => {
        const isSpace = ch === " "
        ...
        const noStaggerDelay =
          phase === "idle" || (phase === "out" && outTick === 0) || (phase === "in" && inTick === 0)
...
function HeroTdy({
  ...
  const rotating = (data.title.rotatingWords ?? []).filter((w) => w.length > 0)
  const hint = data.languagesHint?.trim()
  const heroLangLabel = locale === "en" ? "Multilingual" : "Multilingüe"
  const premiumArs = pricingData.plans.premium
  const heroPriceLine =
    locale === "en"
      ? `from ${locale === "en" ? `USD ${Math.ceil(premiumArs / pricingData.usdArs).toLocaleString("en-US")}` : `$${premiumArs.toLocaleString("es-AR")}`}`
      : `desde $${premiumArs.toLocaleString("es-AR")}`
  const secondaryLabel = locale === "en" ? "View models" : "Ver modelos"

  return (
    <section
      id={heroSectionId}
      className="flex min-h-svh flex-col px-5 pt-10 pb-16 md:px-8"
      style={{ background: theme.background, fontFamily: theme.typography.bodyFont, color: tx.body }}
    >
      <div
        className={`mx-auto flex w-full max-w-6xl flex-1 flex-col ${
          reviewsTopSimple ? "justify-start pt-2 md:justify-center md:pt-0" : "justify-center"
        }`}
      >
        {reviewsTopSimple && rev?.enabled ? (
          rev.url ? (
            <a
              ...
              style={{ color: tx.muted, ...blockRevealStyle(loaded, 20), textDecoration: "none" }}
            >
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={14} fill="#E4B63D" strokeWidth={0} style={{ color: "#E4B63D" }} />
                ))}
              </div>
            ...
            <div className="mx-auto mb-8 mt-0 inline-flex items-center gap-2 text-sm" style={{ color: tx.muted, ...blockRevealStyle(loaded, 20) }}>
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={14} fill="#E4B63D" strokeWidth={0} style={{ color: "#E4B63D" }} />
                ))}
        ...
        {reviewsTopSimple && hint ? (
          <div
            ...
            style={{ color: `${tx.muted}B8`, ...blockRevealStyle(loaded, 30) }}
          >
            <Languages size={12} strokeWidth={1.7} style={{ color: `${tx.muted}B8` }} aria-hidden />
        ...
        >
          <span className={`${HERO_TITLE_LINE_CLASS} mt-0`} style={{ color: tx.heading }}>
            {data.title.line1}
          </span>
          <span className={`${HERO_TITLE_LINE_CLASS} mt-1.5`} style={{ color: tx.heading }}>
            {data.title.line2}
          </span>
          {rotating.length > 0 ? (
            <span className="mt-1.5 block min-h-[1.05em]">
              <HeroStaggerPhrase phrases={rotating} color={theme.accents.softGold} />
            </span>
          ) : null}
        </h1>
        <p
        ...
        >
          <StaggerText text={data.subtitle} revealed={loaded} baseDelayMs={75} wordStepMs={24} />
        </p>
          ...
          >
            <div className="flex items-center gap-2">
              <Languages
                ...
                aria-hidden
              />
              <p className="text-left text-[11px] font-medium leading-snug tracking-wide md:text-xs" style={{ color: tx.muted }}>
                <StaggerText text={hint} revealed={loaded} baseDelayMs={105} wordStepMs={18} />
              </p>
            </div>
            <span
              className="shrink-0 text-[10px] font-semibold tracking-[0.14em] md:text-[11px]"
              style={{ color: theme.accents.softGold }}
              aria-label={locale === "en" ? "Available languages: Spanish and English" : "Idiomas disponibles: español e inglés"}
            >
              ES · EN · PT · FR · +
            </span>
          </div>
          ...
          rev.url ? (
            <a
              ...
              style={{
                ...
                background: "linear-gradient(180deg, #FDF9F1 0%, #FAF3E8 100%)",
                boxShadow:
            ...
            >
              <div className="flex items-center gap-1.5 pr-3" aria-label={`${rev.rating} de 5 estrellas`}>
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={16} fill="#E4B63D" strokeWidth={0} style={{ color: "#E4B63D" }} />
                  ))}
                </div>
                <span className="text-sm font-semibold tabular-nums" style={{ color: "#6E5A45" }}>
                  {rev.rating}
                </span>
              </div>
              <div className="flex flex-col justify-center border-l pl-3 text-left" style={{ borderColor: theme.cardBorder }}>
                <span className="text-[13px] font-bold leading-tight md:text-[14px]" style={{ color: "#6E5A45" }}>
                  {locale === "en" ? "Happy clients" : "Clientes felices"}
                </span>
                <span
                  className="mt-0.5 inline-flex items-center gap-1 text-[11px] font-normal leading-tight md:text-xs"
                  style={{ color: "#6E5A45", textDecoration: "none", opacity: 0.86 }}
                >
                  {rev.linkText}
            ...
            >
              ...
              <div className="flex flex-col justify-center border-l pl-3 text-left" style={{ borderColor: theme.cardBorder }}>
                ...
                <span className="mt-0.5 text-[11px] font-normal leading-tight md:text-xs" style={{ color: "#6E5A45", opacity: 0.76 }}>
                  {rev.source}
                </span>
              </div>
            </div>
        ...
        {reviewsTopSimple ? (
          <div
            ...
            style={blockRevealStyle(loaded, rev?.enabled ? (hint ? 195 : 190) : hint ? 150 : 120)}
          >
            <a
              href={primary?.anchor ?? "#planes"}
              onClick={(event) => handleLocalAnchorClick(event, primary?.anchor ?? "#planes")}
              className="inline-flex min-h-[56px] flex-col items-center justify-center rounded-full px-6 py-3 text-[15px] font-semibold shadow-[0_10px_30px_rgba(120,98,72,0.18)] transition-[transform,box-shadow] duration-200 hover:scale-[1.02] hover:shadow-[0_12px_34px_rgba(120,98,72,0.24)] active:scale-[0.99]"
              style={{ background: theme.buttons.floatingBg, color: theme.buttons.floatingText, textDecoration: "none" }}
            >
              <span>{primary?.text ?? (locale === "en" ? "Create my invitation" : "Crear mi invitación")}</span>
              <span className="mt-0.5 text-[11px] font-medium opacity-90">{heroPriceLine}</span>
            </a>
            <a
              href={secondary?.anchor ?? "#estilos"}
              onClick={(event) => handleLocalAnchorClick(event, secondary?.anchor ?? "#estilos")}
            ...
            <a
              href="#incluye"
              onClick={(event) => handleLocalAnchorClick(event, "#incluye")}
              className="mx-auto mt-7 inline-flex items-center justify-center animate-bounce"
              style={{
                color: `${tx.muted}B8`,
                textDecoration: "none",
              }}
              aria-label={locale === "en" ? "Scroll down" : "Deslizar hacia abajo"}
            >
          ...
          >
            {primary && (
              <CtaLink
                ...
                style={{ background: "#7A5F45", color: "#FFF9F2" }}
              />
            )}
            {secondary && (
              <CtaLink
...
function IncluyeSection({ data, theme }: { data: LandingTdyData["sections"]["incluye"]; theme: LandingTdyTheme }) {
  ...
  return (
    <section id={data.id || "incluye"} className="px-5 pb-20 md:px-8 md:pb-28" style={{ background: theme.background }}>
      <div ref={revealRef} className="mx-auto max-w-6xl">
        <div
          ...
          style={{ borderColor: theme.cardBorder, background: theme.cardBg, ...blockRevealStyle(revealed, 0) }}
        >
          {data.imageSrc ? (
            <Image src={data.imageSrc} alt={data.imageAlt} fill className="object-cover" sizes="(max-width: 1200px) 100vw, 900px" />
          ) : (
            <div className="flex h-full w-full flex-col items-center justify-center gap-2 px-6 text-center" style={{ color: tx.muted }}>
              <Sparkles size={32} strokeWidth={1.25} className="opacity-40" />
              <span className="text-sm">Agregá la imagen en</span>
              <code className="rounded-md bg-black/[0.05] px-2 py-1 text-xs">sections.incluye.imageSrc</code>
            </div>
          )}
        </div>

        <div className="mx-auto mt-10 max-w-3xl">
          <div className="mb-3 flex items-center justify-center" style={blockRevealStyle(revealed, 65)}>
            <h2
              className="text-center text-xl font-semibold tracking-tight md:text-2xl"
              style={{ fontFamily: "var(--font-landing-hero), Georgia, serif", color: tx.heading }}
            >
              <StaggerText text={data.title} revealed={revealed} baseDelayMs={80} wordStepMs={24} />
            </h2>
          </div>
          <p className="mb-5 text-center text-sm leading-relaxed md:text-[15px]" style={{ color: tx.muted, ...blockRevealStyle(revealed, 95) }}>
            <StaggerText text={data.subtitle} revealed={revealed} baseDelayMs={110} wordStepMs={20} />
          </p>
            ...
            {items.map((item, i) => (
              ...
              >
                <span className="mt-0.5 shrink-0 opacity-80">
                  <TdyIcon name={item.icon} size={iconSize} color={theme.accents.iconMuted} />
                </span>
                <span>{item.text}</span>
              </li>
            ))}
...
function ComparativaSection({ data, theme }: { data: LandingTdyData["sections"]["comparativa"]; theme: LandingTdyTheme }) {
  const { revealRef, revealed } = useSectionReveal()
  const c = theme.comparativa
  const tx = theme.text
  const th = data.tableHeaders ?? { feature: "Función", paper: "Papel", digital: "Digital" }
  const bullets: { text: string; subtext?: string; icon: string }[] = data.digital.bullets.map((b) =>
    typeof b === "string" ? { text: b, subtext: "", icon: "sparkles" } : b
  )
  ...
  return (
    ...
    >
      <div ref={revealRef} className="mx-auto max-w-6xl">
        <div className="mb-4 flex justify-center" style={blockRevealStyle(revealed, 0)}>
          <Scale size={26} strokeWidth={1.25} style={{ color: theme.accents.softGold }} aria-hidden />
        </div>
        <h2
        ...
        >
          <StaggerText text={data.title} revealed={revealed} baseDelayMs={55} wordStepMs={22} />
        </h2>
        <p
          className="mx-auto mt-5 max-w-3xl text-center text-base leading-relaxed"
          style={{ color: tx.muted, ...blockRevealStyle(revealed, 100) }}
        >
          <StaggerText text={data.lead} revealed={revealed} baseDelayMs={105} wordStepMs={18} />
        </p>
        <div className="mt-16 grid gap-6 lg:grid-cols-2 lg:gap-8">
          ...
          >
              ...
              >
                <div className="mb-2 flex justify-center" style={blockRevealStyle(revealed, 172)}>
                  <Inbox size={32} strokeWidth={1.6} style={{ color: tx.muted, opacity: 0.82 }} aria-hidden />
                </div>
                <h3 className="text-xl md:text-2xl" style={{ fontFamily: theme.typography.headingFont, color: tx.heading }}>
                  <StaggerText text={data.paper.title} revealed={revealed} baseDelayMs={160} wordStepMs={20} />
                </h3>
                <p className="mt-2 text-sm" style={{ color: tx.muted }}>
                  <StaggerText text={data.paper.guestHint} revealed={revealed} baseDelayMs={182} wordStepMs={14} />
                </p>
              </div>
              <ul className="mt-8 space-y-4">
                {data.paper.lines.map((line) => (
                  ...
                  >
                    <span style={{ color: tx.muted }}>{line.label}</span>
                    <span className="font-medium tabular-nums" style={{ color: tx.body }}>
                      {line.value}
                    </span>
                  </li>
                ))}
              ...
              <div
                ...
                style={{ borderColor: theme.cardBorder }}
              >
                <span className="min-w-0 font-medium" style={{ color: tx.heading }}>
                  <StaggerText text={data.paper.totalLabel} revealed={revealed} baseDelayMs={240} wordStepMs={16} />
                </span>
            ...
            >
              ...
              >
                <div className="mb-2 flex justify-center" style={blockRevealStyle(revealed, 232)}>
                  <Smartphone size={32} strokeWidth={1.6} style={{ color: theme.accents.softGold, opacity: 0.92 }} aria-hidden />
                </div>
                <h3 className="text-xl md:text-2xl" style={{ fontFamily: theme.typography.headingFont, color: tx.heading }}>
                  <StaggerText text={data.digital.title} revealed={revealed} baseDelayMs={225} wordStepMs={20} />
                </h3>
                <p className="mt-2 text-sm" style={{ color: tx.muted }}>
                  <StaggerText text={data.digital.badge} revealed={revealed} baseDelayMs={236} wordStepMs={14} />
                </p>
              </div>
              <p className="mt-3 text-center text-sm font-medium" style={{ color: c.savingsColor }}>
                <StaggerText text={data.digital.savingsLine} revealed={revealed} baseDelayMs={248} wordStepMs={14} />
              </p>
              <div className="mt-8 grid grid-cols-2 gap-x-6 gap-y-5 sm:gap-x-8">
                {bullets.slice(0, 4).map((b) => (
                  <div key={b.text} className="flex items-start gap-2.5">
                    <span className="mt-0.5 inline-flex shrink-0 rounded-lg p-1.5" style={{ background: c.digitalBulletIconBg }}>
                      <TdyIcon name={b.icon} size={16} color={c.digitalBulletIconColor} />
                    </span>
                    <span className="block">
                      <span className="block text-[13px] font-semibold leading-snug" style={{ color: tx.body }}>
                        <StaggerText text={b.text} revealed={revealed} baseDelayMs={270} wordStepMs={12} />
                      </span>
                      {b.subtext ? (
                        <span className="mt-0.5 block text-[11px] leading-snug" style={{ color: tx.muted }}>
                          <StaggerText text={b.subtext} revealed={revealed} baseDelayMs={286} wordStepMs={10} />
                        </span>
                      ) : null}
                    </span>
                  </div>
              ...
              <div className="mt-10 border-t pt-6" style={{ borderColor: c.digitalCardBorder }}>
                <div className="flex flex-row items-center justify-between gap-4">
                  <span className="min-w-0 text-sm font-semibold uppercase tracking-[0.12em]" style={{ color: tx.muted }}>
                    <StaggerText text={data.digital.fromLabel} revealed={revealed} baseDelayMs={285} wordStepMs={14} />
                  </span>
                  <div className="flex shrink-0 flex-col items-end gap-0.5 text-right">
                    <span className="text-3xl font-semibold tabular-nums sm:text-4xl" style={{ ...priceTypeStyle(theme), color: tx.heading }}>
                      {data.digital.fromPrice}
                    </span>
                    {data.digital.fromPriceUsd ? (
                      <span className="text-sm font-semibold tabular-nums" style={{ ...priceTypeStyle(theme), color: c.savingsColor }}>
                        {data.digital.fromPriceUsd}
                      </span>
                    ) : null}
                  </div>
              ...
              <p className="mt-6 text-xs leading-relaxed" style={{ color: tx.caption }}>
                <StaggerText text={data.digital.footnote} revealed={revealed} baseDelayMs={310} wordStepMs={10} />
              </p>
            </div>
          </div>
          <div
            className="mt-16 overflow-visible"
            style={{ background: "transparent", ...blockRevealStyle(revealed, 290) }}
          >
            <p
              className="flex items-center justify-center gap-2 px-2 py-3 text-center text-lg font-semibold md:text-xl"
              style={{ color: tx.heading, fontFamily: "var(--font-landing-hero), Georgia, serif" }}
            >
              <StaggerText text={data.tableTitle} revealed={revealed} baseDelayMs={294} wordStepMs={16} />
            </p>
            <div className="overflow-x-auto px-3 sm:px-5 md:px-7">
              <table className="w-full min-w-[300px] text-left text-sm">
                <thead>
                  <tr>
                    <th className="px-5 py-3 text-left font-semibold" style={{ color: tx.heading }}>
                      <StaggerText text={th.feature} revealed={revealed} baseDelayMs={302} wordStepMs={14} />
                    </th>
                    <th className="px-5 py-3 text-center font-semibold" style={{ color: tx.heading }}>
                      <StaggerText text={th.paper} revealed={revealed} baseDelayMs={316} wordStepMs={14} />
                    </th>
                    <th className="px-5 py-3 text-center font-semibold" style={{ color: tx.heading }}>
                      <StaggerText text={th.digital} revealed={revealed} baseDelayMs={330} wordStepMs={14} />
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {data.tableRows.map((row, ri) => (
                    <tr key={row.feature}>
                      <td className="px-5 py-4" style={{ color: tx.body, ...blockRevealStyle(revealed, 320 + ri * 38) }}>
                        <StaggerText text={row.feature} revealed={revealed} baseDelayMs={322 + ri * 38} wordStepMs={12} />
                      </td>
                      <td className="px-5 py-3" style={blockRevealStyle(revealed, 334 + ri * 38)}>
                        <div className="flex justify-center">
                          <span
                            className="inline-flex h-6 w-6 items-center justify-center rounded-full"
                            style={{ background: c.tablePaperCircleBg }}
                          >
                            <X size={12} strokeWidth={2.75} style={{ color: c.tablePaperIcon }} aria-hidden />
                          </span>
                        </div>
                      </td>
                      <td className="px-5 py-3" style={blockRevealStyle(revealed, 348 + ri * 38)}>
                        <div className="flex justify-center">
                          <span
                            className="inline-flex h-6 w-6 items-center justify-center rounded-full"
                            style={{ background: c.tableDigitalCircleBg }}
                          >
                            <Check size={12} strokeWidth={2.75} style={{ color: c.tableDigitalIcon }} aria-hidden />
                          </span>
                        </div>
                      </td>
                    </tr>
          ...
          <p
            ...
            style={{ color: tx.muted, ...blockRevealStyle(revealed, 360) }}
          >
            <Leaf size={14} aria-hidden />
            <StaggerText text={data.ecoNote} revealed={revealed} baseDelayMs={362} wordStepMs={11} />
          </p>
      </div>
    </section>
  )
...
function CatalogMedia({
  ...
  if (image) {
    return <Image src={image} alt={alt} fill className="object-cover" sizes="360px" />
  }
  return <div className="absolute inset-0 bg-gradient-to-br from-stone-200/90 to-stone-400/50" aria-hidden />
}
...
function EstilosCarousel({
  ...
  return (
    <section id={data.id || "estilos"} className="px-0 py-20 md:py-28" style={{ background: theme.surfaceAlt }}>
      <div ref={revealRef}>
        <div className="mx-auto max-w-6xl px-5 md:px-8">
          <p
            className="text-center text-[11px] font-semibold uppercase tracking-[0.3em]"
            style={{ color: theme.accents.softGold, ...blockRevealStyle(revealed, 0) }}
          >
            <StaggerText text={data.eyebrow} revealed={revealed} baseDelayMs={0} wordStepMs={24} />
          </p>
          <h2
          ...
          >
            <StaggerText text={data.title} revealed={revealed} baseDelayMs={55} wordStepMs={20} />
          </h2>
          <p
            className="mx-auto mt-4 max-w-2xl text-center text-base"
            style={{ color: tx.muted, ...blockRevealStyle(revealed, 105) }}
          >
            <StaggerText text={data.subtitle} revealed={revealed} baseDelayMs={108} wordStepMs={14} />
          </p>
        </div>
        <div className="mt-12 flex snap-x snap-mandatory gap-5 overflow-x-auto scroll-smooth px-5 pb-4 pt-2 md:gap-6 md:px-8 [scrollbar-width:thin]">
          {data.items.map((item, i) => {
            const cardClass =
              "group relative aspect-[4/5] w-[min(88vw,340px)] shrink-0 snap-center overflow-hidden rounded-2xl border shadow-md md:w-[360px]"
            const inner = (
              <>
                <CatalogMedia image={item.image} videoSrc={item.videoSrc} alt={item.titulo} />
                <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/88 via-black/35 to-black/15" aria-hidden />
                <div className="absolute right-3 top-3 rounded-full bg-black/30 p-2 backdrop-blur-sm" aria-hidden>
                  <Sparkles size={17} className="text-white/95" />
                </div>
                <div className="absolute inset-x-0 bottom-0 flex flex-col p-6 pt-20 text-left">
                  <h3 className="text-2xl font-normal text-white" style={{ fontFamily: theme.typography.headingFont }}>
                    <StaggerText text={item.titulo} revealed={revealed} baseDelayMs={cardBaseDelay + i * car.stepBetweenCardsMs + 22} wordStepMs={18} />
                  </h3>
                  <p className="mt-2 text-sm leading-relaxed text-white/88">
                    <StaggerText text={item.descripcion} revealed={revealed} baseDelayMs={cardBaseDelay + i * car.stepBetweenCardsMs + 44} wordStepMs={10} />
                  </p>
                  <span
                    ...
                    style={{ borderColor: b.demoOverlayBorder, color: b.demoOverlayText }}
                  >
                    {data.demoButtonText}
                    <ChevronRight size={13} strokeWidth={2} className="opacity-90" aria-hidden />
                  </span>
                </div>
              </>
            )
            const cardStyle = {
              borderColor: theme.cardBorder,
              ...blockRevealStyle(revealed, cardBaseDelay + i * car.stepBetweenCardsMs),
            }
            if (item.href) {
              return (
                <a key={item.titulo} href={item.href} className={cardClass} style={cardStyle}>
                  {inner}
                </a>
              )
            }
        ...
        <p
          ...
          style={{
            ...
            ...blockRevealStyle(
              revealed,
              cardBaseDelay + data.items.length * car.stepBetweenCardsMs + car.footerAfterCardsExtraMs
            ),
          }}
        >
          <StaggerText text={data.noStylePrompt} revealed={revealed} baseDelayMs={cardBaseDelay + data.items.length * car.stepBetweenCardsMs + car.footerAfterCardsExtraMs + 18} wordStepMs={10} />{" "}
          <a
            href={waNoStyle}
            target="_blank"
            rel="noopener noreferrer"
            ...
            style={{ color: tx.link }}
          >
            <StaggerText text={data.noStyleCta} revealed={revealed} baseDelayMs={cardBaseDelay + data.items.length * car.stepBetweenCardsMs + car.footerAfterCardsExtraMs + 32} wordStepMs={12} />
          </a>
        </p>
      </div>
    </section>
...
function IdiomasSection({ data, theme }: { data: LandingTdyData["sections"]["idiomas"]; theme: LandingTdyTheme }) {
  const { revealRef, revealed } = useSectionReveal()
  const tx = theme.text

  return (
    <section className="px-5 py-20 md:px-8 md:py-28" style={{ background: theme.background }}>
      <div ref={revealRef} className="mx-auto max-w-3xl text-center">
        <div className="mb-4 flex justify-center" style={blockRevealStyle(revealed, 0)}>
          <Languages size={26} strokeWidth={1.25} style={{ color: theme.accents.softGold }} />
        </div>
        <h2
        ...
        <p className="mt-5 text-base leading-relaxed" style={{ color: tx.muted, ...blockRevealStyle(revealed, 100) }}>
          <StaggerText text={data.subtitle} revealed={revealed} baseDelayMs={105} wordStepMs={14} />
        </p>
        <div className="mt-8 flex items-center justify-center" style={blockRevealStyle(revealed, 155)}>
          <span className="h-px w-20" style={{ background: theme.cardBorder }} />
          <Globe2 size={18} strokeWidth={1.8} className="mx-3" style={{ color: theme.accents.softGold }} aria-hidden />
          <span className="h-px w-20" style={{ background: theme.cardBorder }} />
        </div>
          <div className="mx-auto mt-8 grid w-full max-w-4xl grid-cols-3 justify-items-center gap-2.5 sm:gap-3.5 md:gap-4">
            {data.languages.map((lang, i) => (
              ...
              >
                <span className="text-center text-[11px] font-medium leading-snug sm:text-xs md:text-sm" style={{ fontFamily: theme.typography.headingFont, color: tx.heading }}>
                  <StaggerText text={lang.name} revealed={revealed} baseDelayMs={228 + i * 58} wordStepMs={16} />
                </span>
              </div>
            ))}
          </div>
          <div className="mx-auto mt-12 max-w-2xl text-center">
            <p
              ...
              style={{ color: tx.heading, ...blockRevealStyle(revealed, 215 + data.languages.length * 58 + 40) }}
            >
              <StaggerText
                text={data.missingLanguageTitle ?? (tx.inverse ? "¿No ves tu idioma?" : "Don't see your language?")}
                revealed={revealed}
            ...
            <p
              ...
              style={{ color: theme.accents.softGold, ...blockRevealStyle(revealed, 215 + data.languages.length * 58 + 78) }}
            >
              <StaggerText
                text={data.missingLanguageSubtitle ?? "Trabajamos con cualquier idioma del mundo"}
                revealed={revealed}
...
function ProcesoTdy({ data, theme }: { data: LandingTdyData["sections"]["proceso"]; theme: LandingTdyTheme }) {
  const { revealRef, revealed } = useSectionReveal()
  const tx = theme.text
  if (!data.enabled) return null
  return (
    ...
    >
      <div ref={revealRef} className="mx-auto max-w-5xl">
        ...
        >
          <StaggerText text={data.title} revealed={revealed} baseDelayMs={58} wordStepMs={20} />
        </h2>
        <div className="grid gap-12 md:grid-cols-3 md:gap-10">
          {data.steps.map((step, i) => {
            const b0 = 120 + i * 85
            return (
              <div key={step.number + step.title} className="text-center md:text-left">
                <div className="mb-4 flex justify-center md:justify-start" style={blockRevealStyle(revealed, b0)}>
                  <TdyIcon name={step.icon ?? "sparkles"} size={30} color={theme.accents.softGold} />
                </div>
                <h3
                ...
                >
                  <StaggerText text={step.title} revealed={revealed} baseDelayMs={b0 + 42} wordStepMs={16} />
                </h3>
                <p className="mt-3 text-sm leading-relaxed" style={{ color: tx.muted, ...blockRevealStyle(revealed, b0 + 72) }}>
                  <StaggerText text={step.description} revealed={revealed} baseDelayMs={b0 + 74} wordStepMs={10} />
                </p>
              </div>
            )
          })}
...
function PanelSection({ data, theme }: { data: LandingTdyData["sections"]["panel"]; theme: LandingTdyTheme }) {
  const { revealRef, revealed } = useSectionReveal()
  const tx = theme.text
  return (
    <section className="px-5 py-20 md:px-8 md:py-28" style={{ background: theme.background }}>
      <div ref={revealRef} className="mx-auto max-w-6xl">
        <p
          ...
          style={{ color: theme.accents.softGold, ...blockRevealStyle(revealed, 0) }}
        >
          <StaggerText text={data.eyebrow} revealed={revealed} baseDelayMs={0} wordStepMs={22} />
        </p>
        <h2
          className="mt-3 text-center text-3xl font-normal tracking-tight md:text-4xl lg:text-[2.75rem]"
          style={{
        ...
        >
          <StaggerText text={data.title} revealed={revealed} baseDelayMs={52} wordStepMs={20} />
        </h2>
        <p
          className="mx-auto mt-4 max-w-2xl text-center text-base"
          style={{ color: tx.muted, ...blockRevealStyle(revealed, 100) }}
        >
          <StaggerText text={data.subtitle} revealed={revealed} baseDelayMs={104} wordStepMs={14} />
        </p>
        <p
          className="mt-4 flex items-center justify-center gap-2 text-center font-mono text-xs tracking-wide md:text-sm"
          style={{ color: tx.caption, ...blockRevealStyle(revealed, 145) }}
        >
          <LayoutDashboard size={14} aria-hidden />
          <StaggerText text={data.panelUrlLabel} revealed={revealed} baseDelayMs={148} wordStepMs={16} />
        </p>
        <div
          className="relative mx-auto mt-10 max-w-4xl overflow-hidden rounded-2xl border shadow-lg"
          style={{ borderColor: theme.cardBorder, background: theme.cardBg, ...blockRevealStyle(revealed, 195) }}
        >
            {data.imageSrc ? (
              <div className="relative aspect-[16/10] w-full">
                <Image src={data.imageSrc} alt={data.imageAlt} fill className="object-cover object-top" sizes="(max-width: 896px) 100vw, 896px" />
              </div>
            ) : (
                ...
                <div className="grid grid-cols-3 gap-3 md:gap-4">
                  {data.stats.map((s) => (
                    <div
                      ...
                      style={{ borderColor: theme.cardBorder, background: theme.surfaceAlt }}
                    >
                      <p className="text-3xl font-normal tabular-nums" style={{ fontFamily: theme.typography.headingFont, color: tx.heading }}>
                        {s.value}
                      </p>
                      <p className="mt-1 text-xs font-medium uppercase tracking-wider" style={{ color: tx.muted }}>
                        {s.label}
                      </p>
                    </div>
                  ))}
                </div>
                <div className="mt-6 rounded-xl border p-4 md:p-6" style={{ borderColor: theme.cardBorder }}>
                  <div className="mb-4 flex justify-between text-sm">
                    <span className="font-medium" style={{ color: tx.heading }}>
                      {data.guestListTitle}
                    </span>
                    <span style={{ color: tx.muted }}>{data.guestListTotal}</span>
                  </div>
                  <ul className="space-y-2">
                    {data.sampleGuests.map((g) => (
                      <li
                        ...
                        style={{ borderColor: theme.cardBorder }}
                      >
                        <span style={{ color: tx.body }}>{g.name}</span>
                        {g.note ? <span style={{ color: tx.muted }}>{g.note}</span> : null}
                      </li>
                    ))}
                  </ul>
                </div>
                <p className="mt-4 text-center text-xs" style={{ color: tx.caption }}>
                  Screenshot: <code className="rounded bg-black/[0.06] px-1">sections.panel.imageSrc</code>
                </p>
              </div>
            )}
          </div>
        <div className="mt-16 grid gap-14 md:grid-cols-3 md:gap-16">
            {data.features.map((f, i) => (
              <div key={f.title} style={blockRevealStyle(revealed, 265 + i * 72)}>
                <span className="mb-3 inline-flex" style={{ color: theme.accents.softGold }}>
                  <TdyIcon name={f.icon ?? "barChart"} size={24} color={theme.accents.softGold} />
                </span>
                <h3 className="mt-2 text-lg font-normal" style={{ fontFamily: theme.typography.headingFont, color: tx.heading }}>
                  <StaggerText text={f.title} revealed={revealed} baseDelayMs={268 + i * 72} wordStepMs={16} />
                </h3>
                <p className="mt-2 text-sm leading-relaxed" style={{ color: tx.muted }}>
                  <StaggerText text={f.subtitle} revealed={revealed} baseDelayMs={286 + i * 72} wordStepMs={10} />
                </p>
              </div>
            ))}
        </div>
...
function PlanesTdy({
  ...
  if (!data.enabled) return null
  return (
    <section id="planes" className="px-5 py-20 md:px-8 md:py-28" style={{ background: theme.surfaceAlt }}>
      <div ref={revealRef} className="mx-auto max-w-5xl">
        <div className="mb-4 flex justify-center" style={blockRevealStyle(revealed, 0)}>
          <Banknote size={26} strokeWidth={1.35} style={{ color: theme.accents.softGold }} />
        </div>
        <h2
          className="text-center text-3xl font-normal tracking-tight md:text-4xl lg:text-[2.75rem]"
          style={{
        ...
        {data.notice && (
          <p
            ...
            style={{ color: tx.caption, ...blockRevealStyle(revealed, 145) }}
          >
            <StaggerText text={data.notice} revealed={revealed} baseDelayMs={148} wordStepMs={10} />
          </p>
        )}
        <div className="mt-14 grid gap-8 lg:grid-cols-2 lg:items-stretch">
            {data.planes.map((plan, i) => {
              const isPremium = plan.ctaButton === "planPremium" || plan.ctaButton === "plan-premium"
              ...
              return (
                <div
                  key={plan.name}
                  id={plan.ctaButton === "planPremium" ? "plan-premium" : undefined}
                  ...
                  style={{
                    background: theme.cardBg,
                    borderColor: isPremium ? theme.foreground : theme.cardBorder,
                ...
                >
                  {plan.badge && (
                    <span
                      ...
                      style={{ background: b.primaryBg, color: b.primaryText }}
                    >
                      {plan.badge}
                    </span>
                  )}
                  <h3 className="text-2xl font-normal" style={{ fontFamily: theme.typography.headingFont, color: tx.heading }}>
                    <StaggerText text={plan.name} revealed={revealed} baseDelayMs={198 + i * 75} wordStepMs={16} />
                  </h3>
                  <p className="mt-2 text-sm leading-relaxed" style={{ color: tx.muted }}>
                    <StaggerText text={plan.description} revealed={revealed} baseDelayMs={214 + i * 75} wordStepMs={10} />
                  </p>
                  {data.showPrices && (
                    <p
                      className="mt-6 flex flex-wrap items-baseline gap-x-2.5 text-3xl font-semibold sm:text-[2.5rem] lg:text-[2.65rem]"
                      style={{ ...priceTypeStyle(theme), color: tx.heading }}
                    >
                      <span>{plan.price}</span>
                      {plan.priceUsd ? (
                        <span className="text-xl font-medium sm:text-2xl" style={{ ...priceTypeStyle(theme), color: tx.muted }}>
                          · {plan.priceUsd}
                        </span>
                      ) : null}
                    </p>
                  ...
                  {!isUniqueDesign ? (
                    <ul className="mt-8 flex flex-col gap-3">
                      {plan.features.map((f) => (
                        <li key={f} className="flex items-start gap-3 text-sm" style={{ color: tx.body }}>
                          <Check size={16} className="mt-0.5 shrink-0" strokeWidth={2.2} style={{ color: isPremium ? theme.accents.softGold : theme.accents.iconMuted }} />
                          <span>
                            <StaggerText text={f} revealed={revealed} baseDelayMs={232 + i * 75} wordStepMs={9} />
                          </span>
                        </li>
                      ))}
                    </ul>
                  ...
                  {ctaBtn && (
                    <div className={isUniqueDesign ? "mt-8" : "mt-10"}>
                      <CtaLink
                        btn={ctaBtn}
                        waNumber={waNumber}
...
function FaqTdy({
  ...
  return (
    <section id={data.id || "faq"} className="px-5 py-20 md:px-8 md:py-28" style={{ background: theme.surfaceAlt }}>
      <div ref={revealRef} className="mx-auto max-w-3xl">
        ...
        >
          <StaggerText text={data.title} revealed={revealed} baseDelayMs={44} wordStepMs={20} />
        </h2>
        {data.subtitle ? (
          <p className="mx-auto mt-4 max-w-2xl text-center text-base" style={{ color: tx.muted, ...blockRevealStyle(revealed, 88) }}>
            <StaggerText text={data.subtitle} revealed={revealed} baseDelayMs={92} wordStepMs={12} />
          </p>
        ) : null}
        <div className="mt-12">
          {data.items.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className="text-sm font-semibold leading-relaxed md:text-[15px]" style={{ color: tx.heading }}>
                    {item.question}
                  </span>
                  <span
                ...
                >
                  <p className="pb-5 pr-2 text-sm leading-relaxed md:text-[15px]" style={{ color: tx.muted }}>
                    {item.answer}
                  </p>
                </div>
              </div>
...
export default function LandingTdyPage({
  ...
  const footerTheme = useMemo<LandingTdyTheme>(
    () =>
      mode === "landing2"
        ? {
            ...theme,
  ...
  const heroSectionId = mode === "landing2" ? "hero-landing2" : undefined
  ...
  }, [floatingCta, mode, heroSectionId])
    ...
    >
      ...
      {sections.hero.enabled && (
        <HeroTdy
          ...
          waNumber={wa}
          locale={showLang ? locale : "es"}
          reviewsTopSimple={mode === "landing2"}
      ...
      <EstilosCarousel data={sections.estilos} theme={mode === "landing2" ? theme : theme} waNumber={wa} />
      {mode === "landing2" ? (
        <>
          <PanelSection data={sections.panel} theme={theme} />
          <IdiomasSection data={sections.idiomas} theme={themeAlt} />
          <ProcesoTdy data={sections.proceso} theme={themeAlt} />
          <PlanesTdy data={sections.servicio} theme={theme} buttons={ctaButtons} waNumber={wa} />
      ...
      <div style={{ marginBottom: fabScrollGap }}>
        <LandingFooter theme={footerTheme} />
      </div>
      {floatingCtaForMode ? <FloatingCta data={floatingCtaForMode} theme={theme} /> : null}
    </main>
  )
}
