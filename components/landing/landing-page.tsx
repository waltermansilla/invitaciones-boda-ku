...
interface LandingData {
  ...
  sections: {
    ...
    muestras: {
      ...
      title: string
      description: string
      items: { tipo: string; slug: string; titulo: string; etiqueta: string; accentColor: string }[]
    }
    servicio: {
      enabled: boolean
      title: string
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {data.title.normal}
          <span style={{ fontWeight: 600 }}>{data.title.highlight}</span>
        </h1>
        <p
...
function MuestrasSection({
  ...
  return (
    <section id="muestras" className="px-6 py-20 md:py-28" style={{ background: theme.background }}>
            ...
            {data.planes.map((plan, i) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-lg font-medium tracking-wide" style={{ color: theme.foreground }}>
                    {plan.name}
                  </h3>
          ...
          <div className="flex flex-col gap-10 md:flex-row md:gap-8">
            {data.steps.map((step, i) => (
              <div key={i} className="flex-1 text-center">
                <span className="mb-3 inline-block text-3xl font-extralight" style={{ color: theme.accent + "40" }}>
                  {step.number}
                </span>
  ...
  return (
    <main style={{ background: theme.background }}>
      {sections.hero.enabled && (
        <HeroSection data={sections.hero} theme={theme} buttons={ctaButtons} waNumber={whatsapp.number} />
      )}
      {sections.muestras.enabled && <MuestrasSection data={sections.muestras} theme={theme} />}
      {sections.servicio.enabled && (
        <ServicioSection data={sections.servicio} theme={theme} buttons={ctaButtons} waNumber={whatsapp.number} />
      )}
      {sections.proceso.enabled && (
        <ProcesoSection data={sections.proceso} theme={theme} buttons={ctaButtons} waNumber={whatsapp.number} />
      )}
      {sections.faq.enabled && <FaqSection data={sections.faq} theme={theme} />}
      {sections.ctaFinal.enabled && (
        <CtaFinalSection data={sections.ctaFinal} theme={theme} buttons={ctaButtons} waNumber={whatsapp.number} />
      )}
